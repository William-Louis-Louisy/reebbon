...
$sourceDirectory = Join-Path $repositoryRoot "modules\reebbon-cbr\native"
...
foreach ($abi in @("armeabi-v7a", "arm64-v8a", "x86", "x86_64")) {
  $buildDirectory = Join-Path $repositoryRoot "android\.cxx\cbr-native-$abi"
  $testFlag = if ($abi -eq "arm64-v8a" -or $abi -eq "x86_64") { "ON" } else { "OFF" }
  & $cmake `
    -S $sourceDirectory `
    -B $buildDirectory `
    -G Ninja `
  ...
  & $cmake --build $buildDirectory --target reebbon-cbr
  if ($LASTEXITCODE -ne 0) {
  ...
  if ($testFlag -eq "ON") {
    ...
    & $cmake --build $buildDirectory --target reebbon-cbr-fixture-runner
    if ($LASTEXITCODE -ne 0) {
      throw "Native fixture runner build failed for $abi"
    }
  }

  $library = Join-Path $buildDirectory "libreebbon-cbr.so"
  $strippedLibrary = Join-Path $buildDirectory "libreebbon-cbr-stripped.so"
