...
ExtractionResult extractToDirectory(const std::string& sourcePath,
  ...
  const std::uint64_t archiveBytes = fs::file_size(source, filesystemError);
  if (filesystemError) {
    throw ExtractionError("ERR_CBR_SOURCE_ACCESS",
                          "The RAR source size could not be read");
  }
  if (archiveBytes > limits.maxArchiveBytes) {
    throw ExtractionError("ERR_CBR_ARCHIVE_SIZE_LIMIT",
                          "The RAR archive exceeds the input size limit");
  }
  ...
  try {
    ...
    while (true) {
      ...
      const int processResult = RARProcessFileW(
          archive.get(), RAR_EXTRACT, destinationWide.data(), nullptr);
