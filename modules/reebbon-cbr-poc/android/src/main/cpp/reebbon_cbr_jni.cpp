...
Java_expo_modules_reebboncbrpoc_ReebbonCbrPocModule_extractNative(
...
Java_expo_modules_reebboncbrpoc_ReebbonCbrPocModule_cleanupNative(
    JNIEnv* environment, jobject, jstring destinationPath) {
  if (destinationPath == nullptr) {
    throwNativeError(environment, "ERR_CBR_INVALID_ARGUMENT", "A path is required");
    return;
  }
  UtfChars destination(environment, destinationPath);
  ...
  try {
    reebbon::cbr::cleanupDirectory(destination.get());
  } catch (const reebbon::cbr::ExtractionError& error) {
    throwNativeError(environment, error.code(), error.what());
  }
}
